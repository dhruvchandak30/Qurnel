...
const generatePayload = (priority: Priority): Payload => {
    const timeToProcess = {
...
const PopulateDataInRedis = () => {
    const high = Array.from({ length: randomInt(5, 15) }, () =>
        generatePayload('HIGH'),
    );
    const medium = Array.from({ length: randomInt(5, 15) }, () =>
        generatePayload('MEDIUM'),
    );
    const low = Array.from({ length: randomInt(5, 15) }, () =>
        generatePayload('LOW'),
    );
...
export default PopulateDataInRedis;
